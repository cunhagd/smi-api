import {
  ...
  export class NoticiasService {
    ...
    async findAll(filterDto: FilterNoticiasDto) {
      ...
      this.logger.debug(
        `Filtros: from=${fromFormatted}, to=${toFormatted}, date=${dateFormatted}, before=${beforeFormatted}, after=${afterFormatted}, utilidade=${utilidade}, estrategica=${estrategica}, tema=${tema}, titulo=${titulo}, portal=${portal}, avaliacao=${avaliacao}`,
      );
    ...
    async update(id: number, updateDto: UpdateNoticiaDto) {
      ...
      try {
        ...
        if (updateDto.avaliacao !== undefined) {
          fieldsToUpdate.avaliacao = (updateDto.avaliacao === null ? null : updateDto.avaliacao) as string | undefined;
        }
